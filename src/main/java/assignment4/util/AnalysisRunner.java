...
public class AnalysisRunner {
	...
	public void runValueIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			int timeint=(int) (System.nanoTime()-startTime)/1000000;
			AnalysisAggregator.addMillisecondsToFinishValueIteration(timeint);

			AnalysisAggregator.sumTimeVI(timeint);
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runPolicyIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			int timeint=(int) (System.nanoTime()-startTime)/1000000;
			AnalysisAggregator.addMillisecondsToFinishPolicyIteration(timeint);
			AnalysisAggregator.sumTimePI(timeint);
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runQLearning(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			int timeint=(int)(System.nanoTime()-startTime)/1000000;
			AnalysisAggregator.addMillisecondsToFinishQLearning(timeint);

			AnalysisAggregator.sumTimeQ(timeint);
